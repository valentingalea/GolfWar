...
camera.position.set(0, eyeHeight, 4.2);
...
const ground = new THREE.Mesh(
  ...
  new THREE.MeshStandardMaterial({ color: 0x111824, roughness: 0.95, metalness: 0.05 })
);
...
scaleCylinder.position.set(-1.4, 0.9, 0.6);
scene.add(scaleCylinder);

const cannon = new THREE.Group();
cannon.position.set(2.2, 0, 0);
cannon.scale.set(1.5, 1.5, 1.5);
scene.add(cannon);
...
const base = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.6, 1.2), baseMaterial);
base.position.set(0, 0.45, 0.1);
cannon.add(base);
...
camera.lookAt(new THREE.Vector3(-1.4, 1.0, 0.2));
